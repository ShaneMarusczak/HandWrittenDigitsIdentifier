...
"",document.getElementById("guessConfidence").textContent="Confidence too low to guess"):(90<100*c[0]?(document.getElementById("guess").textContent=c[1],document.getElementById("guessConfidence").textContent="High Confidence (>90%)"):(document.getElementById("guess").textContent=c[1],document.getElementById("guessConfidence").textContent="Low Confidence (70-90%)"),50<100*a[0]&&(document.getElementById("guess").textContent=c[1]+" or "+a[1]))}function v(b){if(l){var c=$jscomp.makeIterator(f(b.target));
b=c.next().value;c=c.next().value;e[b][c].increaseGreyScale(1);e[b][c].neighbors.forEach(function(a){e[a[0]][a[1]].increaseGreyScale(.7)})}}function w(){document.getElementById("guess").textContent="";document.getElementById("guessConfidence").textContent="";for(var b=0;28>b;b++)for(var c=0;28>c;c++)e[c][b].greyScaleValue=0,document.getElementById("cell-"+b+"-"+c).style.backgroundColor="white"}var l=!1,e=[],x=document.getElementById("gameBoard_UI"),k=function(b,c){this.x=b;this.y=c;this.greyScaleValue=
0;this.neighbors=[]};k.prototype.increaseGreyScale=function(b){this.greyScaleValue+=b;1<this.greyScaleValue&&(this.greyScaleValue=1);this.setBackGround()};k.prototype.setBackGround=function(){var b="rgb(30, 30, 30,"+this.greyScaleValue+")";document.getElementById("cell-"+this.x+"-"+this.y).style.backgroundColor=b};k.prototype.resetBackGround=function(){document.getElementById("cell-"+this.x+"-"+this.y).style.backgroundColor="white"};k.prototype.setNeighbors=function(){for(var b=[-1,0,1],c=$jscomp.makeIterator(b),
a=c.next();!a.done;a=c.next())for(var a=a.value,d=$jscomp.makeIterator(b),e=d.next();!e.done;e=d.next())e=e.value,!g(this.x+a,this.y+e)||0===a&&0===e||this.neighbors.push([this.x+a,this.y+e])};(function(){document.getElementById("start").addEventListener("click",q);document.getElementById("clear").addEventListener("click",w);document.body.onmousedown=d;document.body.onmousemove=d;document.body.onmouseup=d;for(var b=0;28>b;b++){e.push([]);var c=document.createElement("div");c.id="col-"+b;c.classList.add("col");
x.appendChild(c);c.draggable=!1;c.ondragstart=function(){return!1};for(var a=0;28>a;a++){var f=new k(b,a);e[b].push(f);f.setNeighbors();f=document.createElement("div");f.id="cell-"+b+"-"+a;f.classList.add("cell");c.appendChild(f);f.addEventListener("mouseover",v)}}})()})();
