...
(function(){function d(b){l=void 0===b.buttons?1===b.which:1===b.buttons}function f(b){return[b.id.split("-")[1],b.id.split("-")[2]]}function g(b,c){return 0<=b&&28>b&&0<=c&&28>c}function q(){for(var b=0;14>b;b++)r();for(b=0;14>b;b++)t();for(var b=[],c=0;28>c;c++)for(var a=0;28>a;a++)b.push([e[a][c].greyScaleValue]);b="https://shnpln252.pythonanywhere.com/digits?digits="+JSON.stringify(b);fetch(b,{method:"get",mode:"cors"}).then(function(a){return a.text()}).then(u)}function t(){for(var b,c,a=!1,
d=!1,h=0;28>h;h++){for(var f=0;28>f;f++)if(0<e[f][h].greyScaleValue){b=h;a=!0;break}if(a)break}for(a=27;0<=a;a--){for(h=27;0<=h;h--)if(0<e[h][a].greyScaleValue){c=28-a;d=!0;break}if(d)break}b!==c&&(b>c&&m(-1),b<c&&m(1))}function r(){for(var b,c,a=0;28>a;a++)if(e[a].some(function(a){return 0<a.greyScaleValue})){b=a;break}for(a=27;0<=a;a--)if(e[a].some(function(a){return 0<a.greyScaleValue})){c=28-a;break}b!==c&&(b>c&&n(-1),b<c&&n(1))}function p(){for(var b=[],c=0;28>c;c++)for(var a=0;28>a;a++)0<e[c][a].greyScaleValue&&
b.push(e[c][a]);return b}function n(b){var c=p();-1===b?c.forEach(function(a){g(a.x+b,a.y)&&(e[a.x+b][a.y].greyScaleValue=a.greyScaleValue,e[a.x+b][a.y].setBackGround());a.greyScaleValue=0;a.resetBackGround()}):c.slice().reverse().forEach(function(a){g(a.x+b,a.y)&&(e[a.x+b][a.y].greyScaleValue=a.greyScaleValue,e[a.x+b][a.y].setBackGround());a.greyScaleValue=0;a.resetBackGround()})}function m(b){var c=p();-1===b?c.forEach(function(a){g(a.x,a.y+b)&&(e[a.x][a.y+b].greyScaleValue=a.greyScaleValue,e[a.x][a.y+
b].setBackGround());a.greyScaleValue=0;a.resetBackGround()}):c.slice().reverse().forEach(function(a){g(a.x,a.y+b)&&(e[a.x][a.y+b].greyScaleValue=a.greyScaleValue,e[a.x][a.y+b].setBackGround());a.greyScaleValue=0;a.resetBackGround()})}function u(b){document.getElementById("guess").textContent=b}function v(b){if(l){var c=$jscomp.makeIterator(f(b.target));b=c.next().value;c=c.next().value;e[b][c].increaseGreyScale(1);e[b][c].neighbors.forEach(function(a){e[a[0]][a[1]].increaseGreyScale(.7)})}}function w(){for(var b=
0;28>b;b++)for(var c=0;28>c;c++)e[c][b].greyScaleValue=0,document.getElementById("cell-"+b+"-"+c).style.backgroundColor="rgb(201, 201, 201, 0.4)"}var l=!1,e=[],x=document.getElementById("gameBoard_UI"),k=function(b,c){this.x=b;this.y=c;this.greyScaleValue=0;this.neighbors=[]};k.prototype.increaseGreyScale=function(b){this.greyScaleValue+=b;1<this.greyScaleValue&&(this.greyScaleValue=1);this.setBackGround()};k.prototype.setBackGround=function(){var b="rgb(30, 30, 30,"+this.greyScaleValue+")";document.getElementById("cell-"+
...
q);document.getElementById("clear").addEventListener("click",w);document.body.onmousedown=d;document.body.onmousemove=d;document.body.onmouseup=d;for(var b=0;28>b;b++){e.push([]);var c=document.createElement("div");c.id="col-"+b;c.classList.add("col");x.appendChild(c);c.draggable=!1;c.ondragstart=function(){return!1};for(var a=0;28>a;a++){var f=new k(b,a);e[b].push(f);f.setNeighbors();f=document.createElement("div");f.id="cell-"+b+"-"+a;f.classList.add("cell");c.appendChild(f);f.addEventListener("mouseover",
v)}}})()})();
